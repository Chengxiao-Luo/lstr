...
def main():
    ...
    print(sorted(backbone_state_dict.keys()))
    ...
    for item in data:
        ...
        def forward_data(model, data):
            ...
            while start_idx < num_clip:
                with torch.no_grad():
                    ...
                    feat = model.backbone.forward(part).cpu()
                    print(feat.shape)
                    results.append(feat)
        ...
        save_path = osp.join("data", "thumos14", "rgb_feat", "val", osp.basename(frame_dir) + '.npy')
