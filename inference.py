...
def main(cfg):
    ...
    with torch.no_grad():
        for session_idx, session in enumerate(cfg.DATA.TEST_SESSION_SET):
            visual_inputs = np.load(osp.join(cfg.DATA.DATA_ROOT, cfg.INPUT.VISUAL_FEATURE, session + '.npy'),
                                    mmap_mode='r')
            motion_inputs = np.load(osp.join(cfg.DATA.DATA_ROOT, cfg.INPUT.MOTION_FEATURE, session + '.npy'),
                                    mmap_mode='r')
            ...
            for work_start, work_end in zip(range(0, target.shape[0] + 1),
                ...
                memory_key_padding_mask = torch.as_tensor(memory_key_padding_mask.astype(np.float32)).unsqueeze(0).to(
                    device)
            ...
            for i, pred_score in enumerate(pred_scores):
                pred_label = list(np.where(pred_score > threshold))
                pred_label_list = []
                for label in pred_label:
                    pred_label_list.append(label[0].tolist())
                pred_labels.append(pred_label_list)

            print(pred_labels)
            save_path = osp.join("data", "thumos14", "labels", "val", session + ".npz")
            os.makedirs(osp.dirname(save_path), exist_ok=True)
            np.savez(save_path, pred_labels=pred_labels)
